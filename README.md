...
Live Demo: https://YOURUSERNAME.github.io/world-radio/ (replace after deploy)

PWA Ready Offline Shell No Build License: MIT
...
Android / Desktop Chrome / Edge: Open the deployed URL → banner Install World Radio? → Install. Or menu → Install app.

iPhone / iPad (Safari only): Open URL in Safari → Share button (square + arrow) → Add to Home Screen → Add. Opens standalone, no address bar.
...
Create new public repo world-radio
Upload these 7 files to the root of the repo (not inside a folder):
index.html
...
Wait ~1 min → https://YOURUSERNAME.github.io/world-radio/
CLI:
...
Option B — Pure iPad only: Install a-Shell mini → move folder to its Documents → run:
...
API: Radio Browser community directory. Tries de1, nl1, at1, fr1 mirrors with 4s timeout fallback.
...
Streams via Radio Browser — free, open, community-maintained. All station logos/favicons belong to their owners.
